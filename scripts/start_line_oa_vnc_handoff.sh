...
# This process grants interactive control of the authenticated browser. It is
...
cleanup() { kill "${expiry_pid:-}" "${tunnel_pid:-}" "${caddy_pid:-}" "${websockify_pid:-}" "${vnc_pid:-}" "${chrome_pid:-}" "${xvfb_pid:-}" 2>/dev/null || true; }
on_signal() { printf 'Handoff revoked; closing tunnel and browser.\n' >&2; exit 0; }
trap cleanup EXIT
...
(
  sleep "$handoff_ttl_seconds"
  printf 'Handoff TTL expired; revoking tunnel and closing browser.\n' >&2
  kill -TERM "$$" 2>/dev/null || true
) & expiry_pid=$!
printf 'Login-only handoff expires in %ss; Ctrl-C revokes it early and closes Chromium.\n' "$handoff_ttl_seconds"
wait "$chrome_pid"
