...
4. If `uv` or `Xvfb` is missing, or package download is blocked, report exactly that host-level blocker and its command output. For VNC/noVNC handoff dependencies, do not use `sudo`; when `apt-get` exists, attempt the normal unprivileged `apt-get update` and install commands regardless of `id -u`, then report their real result. If the host grants package capability to the running identity, provisioning succeeds; otherwise the permission error is the verified blocker. Never weaken network bindings.
