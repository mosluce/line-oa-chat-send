...
4. If `uv` or `Xvfb` is missing, or package download is blocked, report exactly that host-level blocker and its command output. For VNC/noVNC handoff dependencies, first check `id -u`: when already root on an apt-based host, the handoff helper may use `apt-get` directly to install its declared dependencies; it must never invoke `sudo`. When non-root, report the missing packages without escalating privileges or weakening network bindings.
