...
- ~~What is the shortest safe grace window before the first DNS lookup?~~ **Resolved: 5s**, the smallest swept value that hit on every run. Applied as the default; measured effect is 25.2s → 10.3s on the common path. Tunnel registration (4.45s) is now the largest single component and is out of scope by decision. See `results.md`.
- ~~Which pole dominates end to end — the session cold start or the tunnel?~~ **Neither.** Attach mode removed the session from the handoff path, and the tunnel term is now 4.45s of a 10.3s script. The dominant cost turned out to be outside the script entirely, in agent turns.
- ~~How much of the end-to-end interval sits in agent turns rather than in the script?~~ **Resolved: most of it.** Measured on the deployed agent: 18.58s before the script ran, ~10.3s of script, ~6.9s after — agent turns are 2.5× the script. The premise that the script was the bottleneck does not hold, and further script optimization has poor returns. See `results.md`.
